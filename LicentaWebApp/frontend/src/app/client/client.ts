...
export class Client extends ClientBase {
     ...
     * @return Success
     */
    myRepo(id: string): Observable<Repository[]> {
        let url_ = this.baseUrl + "/api/Base/repos/myRepo/{id}";
        if (id === undefined || id === null)
            throw new Error("The parameter 'id' must be defined.");
        url_ = url_.replace("{id}", encodeURIComponent("" + id));
        url_ = url_.replace(/[?&]$/, "");
        ...
        return _observableFrom(this.transformOptions(options_)).pipe(_observableMergeMap(transformedOptions_ => {
            ...
            return this.processMyRepo(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processMyRepo(response_ as any);
                } catch (e) {
    ...
    protected processMyRepo(response: HttpResponseBase): Observable<Repository[]> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
